...
export function DataExplorer({ module, records }: { module: string; records: RecordValue[] }) {
  ...
  return <section className="ck-card overflow-hidden">
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[760px] border-collapse text-left text-sm"><thead className="bg-[#f8f5ef] text-[10px] uppercase tracking-[.12em] text-slate-500"><tr>{columns.map((column) => <th className="border-b px-4 py-3 font-bold" key={column}>{column.replaceAll(/([A-Z])/g," $1")}</th>)}</tr></thead><tbody>{filtered.map((record,index) => <tr className="transition hover:bg-amber-50/45" key={String(record.id ?? index)}>{columns.map((column,position) => <td className={`border-b px-4 py-4 ${position === 0 ? "font-semibold text-[#755714]" : ""}`} key={column}>{readable(record[column])}</td>)}</tr>)}</tbody></table>{!filtered.length && <div className="p-12 text-center text-sm text-slate-500">No records match this view.</div>}</div>
