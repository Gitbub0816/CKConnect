...
import { CalendarWorkbench } from "@/components/calendar-workbench";
...
function Pill({
  ...
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${styles[tone]}`}
    >
      {children}
    </span>
  );
  ...
  return (
    <div className="space-y-4">
      {(data.records ?? []).map((item) => (
        <article
          className="ck-card grid gap-5 p-5 xl:grid-cols-[1fr_420px]"
          key={String(item.id)}
        >
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <Pill
                tone={
                  ["URGENT", "HIGH"].includes(String(item.priority))
                    ? "danger"
                    : "warning"
                }
              >
              ...
              <Pill tone={item.status === "CLOSED" ? "success" : "neutral"}>
                {String(item.status)}
              </Pill>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              {String(item.description ?? "No issue description was provided.")}
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-lg bg-[#f8f5ef] p-3">
                ...
                <div className="mt-1 text-sm font-semibold">
                  {Number(item.ageHours)} hours
                </div>
              </div>
            </div>
          </div>
          <form
            action={updateCaseWorkflow}
          ...
          >
            ...
            />
            <input name="entityId" type="hidden" value={String(item.id)} />
            ...
            <label className="mt-3 block text-xs font-semibold text-slate-600">
              Resolution or customer-facing update
              <textarea
                className="ck-input mt-2 min-h-24 py-3"
                defaultValue={String(item.resolution ?? "")}
                name="resolution"
                placeholder="Required to resolve or close the case."
              />
            </label>
  ...
  return (
    <div className="space-y-4">
      {(data.records ?? []).map((campaign) => (
        <article className="ck-card overflow-hidden" key={String(campaign.id)}>
          <div className="grid gap-5 p-5 lg:grid-cols-[1fr_repeat(3,130px)]">
            <div>
              <div className="flex items-center gap-2">
                <Megaphone className="text-[#9b7420]" size={18} />
                <Pill
              ...
              <p className="mt-1 text-sm text-slate-500">
                {String(campaign.type)} campaign · {String(campaign.audience)}
              </p>
            </div>
            <div>
              <div className="text-[10px] font-bold uppercase text-slate-500">
              ...
              <div className="mt-2 text-xl font-semibold">
                {Number(campaign.members).toLocaleString()}
              </div>
            </div>
            <div>
              <div className="text-[10px] font-bold uppercase text-slate-500">
              ...
              <div className="mt-2 text-xl font-semibold">
                {Number(campaign.responseRate).toFixed(1)}%
              </div>
            </div>
            <div>
              <div className="text-[10px] font-bold uppercase text-slate-500">
          ...
          <form
            ...
            className="grid gap-3 border-t bg-[#f8f5ef] p-5 lg:grid-cols-[180px_1fr_1.4fr_auto] lg:items-end"
          >
            <input
              name="organizationSlug"
              type="hidden"
              value={organizationSlug}
            />
            <input name="entityId" type="hidden" value={String(campaign.id)} />
            <label className="text-xs font-semibold text-slate-600">
              Audience
              <select className="ck-input mt-2" name="audience">
                <option value="ALL_CONTACTS">All emailable contacts</option>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                name="command"
                type="submit"
                value={campaign.status === "ACTIVE" ? "PAUSE" : "COMPLETE"}
              >
                <CirclePause size={14} />
                {campaign.status === "ACTIVE" ? "Pause" : "Complete"}
              </button>
            </div>
          </form>
        </article>
  ...
  return (
    <div className="space-y-3">
      {(data.records ?? []).map((event) => (
        <article
          className="ck-card grid gap-5 p-5 xl:grid-cols-[1fr_520px]"
          key={String(event.id)}
        >
          <div>
            <div className="flex items-center gap-2">
              <CalendarClock className="text-[#9b7420]" size={18} />
              <Pill
                tone={
            ...
            <p className="mt-1 text-sm text-slate-500">
              ...
              {String(event.timeRange)} ·{" "}
              {String(event.location ?? "No location")}
            </p>
            <p className="mt-3 text-xs text-slate-500">
              {Number(event.attendees)} attendee(s) · {String(event.type)}
            </p>
          </div>
          <form
            action={updateCalendarCommitment}
            className="grid gap-3 rounded-lg border bg-slate-50 p-4 sm:grid-cols-[1fr_140px_auto] sm:items-end"
          >
            <input
              name="organizationSlug"
              type="hidden"
            ...
            <label className="text-xs font-semibold text-slate-600">
              ...
              <input
                className="ck-input mt-2"
                name="followUp"
                placeholder="Send recap and proposal"
              />
            </label>
            <label className="text-xs font-semibold text-slate-600">
              Due
        ...
        {(data.records ?? []).map((message) => (
          ...
          >
            <div>
              ...
              <h3 className="mt-3 font-semibold">{String(message.subject)}</h3>
    ...
    <div className="space-y-4">
      <section className="grid gap-4 xl:grid-cols-[1fr_320px]">
        <div className="space-y-3">
          {(data.runs ?? []).map((run) => (
            <article
              className="ck-card grid gap-5 p-5 lg:grid-cols-[1fr_auto]"
              key={String(run.id)}
            >
              <div>
                <div className="flex items-center gap-2">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Check date{" "}
                  {new Date(String(run.checkDate)).toLocaleDateString()} ·{" "}
                  {Number(run.employeeCount)} employee(s)
                </p>
                <div className="mt-4 flex flex-wrap gap-5 text-sm">
                  <span>
                    Gross{" "}
                    <strong>{formatCurrency(Number(run.grossPay))}</strong>
                  </span>
                  <span>
                    Net <strong>{formatCurrency(Number(run.netPay))}</strong>
                  </span>
                  <span>
                    Employer cost{" "}
                    <strong>{formatCurrency(Number(run.employerCost))}</strong>
                  </span>
                </div>
              </div>
              <form action={advancePayrollRun} className="flex items-end gap-2">
                <input
                  name="organizationSlug"
                  type="hidden"
                  value={organizationSlug}
                />
                <input name="entityId" type="hidden" value={String(run.id)} />
        ...
        <aside className="ck-card h-fit p-5">
          <div className="ck-eyebrow">Provider control</div>
          <h3 className="mt-2 text-xl font-semibold">
            {String(connection?.provider ?? "Not connected")}
          </h3>
          <p className="mt-2 text-sm text-slate-500">
            {String(connection?.mode ?? "Choose embedded or connected payroll")}{" "}
            · {String(connection?.status ?? "DISCONNECTED")}
          </p>
          <div className="mt-5 rounded-lg bg-amber-50 p-4 text-xs leading-5 text-amber-900">
            ClearKey owns approvals, evidence, and accounting handoff. Tax
            calculation, filing, remittance, and direct deposit remain with the
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="ck-card overflow-hidden">
          <div className="border-b p-5">
            <h3 className="font-semibold">Submitted time</h3>
          </div>
          <div className="divide-y">
            {(data.time ?? []).map((entry) => (
              ...
              >
                <div>
                  <strong>{String(entry.employee)}</strong>
                  <p className="mt-1 text-xs text-slate-500">
                    {new Date(String(entry.date)).toLocaleDateString()} ·{" "}
                    {Number(entry.regular)} regular · {Number(entry.overtime)}{" "}
                    overtime
                  </p>
                </div>
                {entry.status === "SUBMITTED" ? (
                  ...
                  <Pill tone="success">{String(entry.status)}</Pill>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="ck-card overflow-hidden">
          <div className="border-b p-5">
            <h3 className="font-semibold">Time-off decisions</h3>
          </div>
          <div className="divide-y">
            {(data.timeOff ?? []).map((request) => (
              <div
                className="flex items-center justify-between gap-4 p-5"
                key={String(request.id)}
              >
                <div>
                  <strong>{String(request.employee)}</strong>
                  <p className="mt-1 text-xs text-slate-500">
                    {String(request.type)} · {Number(request.hours)} hours ·{" "}
                    {new Date(String(request.startsOn)).toLocaleDateString()}
                  </p>
                </div>
                {request.status === "PENDING" ? (
                  <form action={reviewTimeOff} className="flex gap-2">
                    <input
                      name="organizationSlug"
                      type="hidden"
                      value={organizationSlug}
...
export function ServiceWorkbench({
  ...
  if (module === "cases")
    return <CaseWorkbench data={data} organizationSlug={organizationSlug} />;
  if (module === "campaigns")
    return (
      <CampaignWorkbench data={data} organizationSlug={organizationSlug} />
    );
  if (module === "calendar")
    return (
      <CalendarWorkbench
  ...
  if (module === "email")
    return <EmailWorkbench data={data} organizationSlug={organizationSlug} />;
  if (module === "payroll")
    return <PayrollWorkbench data={data} organizationSlug={organizationSlug} />;
  return null;
}
