...
function Block({ block, organizationSlug }: { block: PortalBlock; organizationSlug: string }) {
  if (block.type === "hero") return (
    <section className={`portal-hero portal-hero-${block.layout ?? "split"}`}>
      <div className="portal-hero-copy">
        {block.eyebrow && <div className="portal-eyebrow">{block.eyebrow}</div>}
        <h1>{block.title}</h1><p>{block.body}</p>
        <div className="portal-actions"><a className="portal-button" href="#contact">{block.primaryAction}<ArrowRight size={16}/></a>{block.secondaryAction && <Link className="portal-button portal-button-secondary" href={`/c/${organizationSlug}/login`}>{block.secondaryAction}</Link>}</div>
      </div>
  ...
  if (block.type === "payment") return <section className="portal-cta" id={block.id}><div><span>Secure payments</span><h2>{block.title}</h2><p>{block.body}</p></div><Link className="portal-button portal-button-light" href={`/c/${organizationSlug}/login`}>{block.action ?? "Pay an invoice"}<ArrowRight size={16}/></Link></section>;
  ...
  if (block.type === "portal") return <section className="portal-section" id={block.id}><div className="portal-section-heading"><span>Customer portal</span><h2>{block.title}</h2></div><p>{block.body}</p><Link className="portal-button mt-8" href={`/c/${organizationSlug}/login`}>{block.action ?? "Open portal"}<ShieldCheck size={16}/></Link></section>;
  ...
  if (block.type === "resourceLinks") return <section className="portal-section" id={block.id}><div className="portal-section-heading"><span>Resources</span><h2>{block.title}</h2></div><div className="portal-resources">{block.items?.map((item) => <Link href={item.action === "booking" ? "#booking" : `/c/${organizationSlug}/login`} key={item.label}>{item.action === "payment" ? <FileText size={20}/> : <CalendarDays size={20}/>}<span>{item.label}</span><ArrowRight size={17}/></Link>)}</div></section>;
...
export function PortalRenderer({
  ...
  return (
    <main className="portal-root" style={style}>
      <div className="portal-shell">
        <nav className="portal-nav">
          <Link className="portal-brand" href={`/p/${organization.slug}`}>{theme?.logoUrl ? (
            // Customer logos can be hosted on arbitrary verified domains.
            // eslint-disable-next-line @next/next/no-img-element
            <img alt={`${organization.name} logo`} src={theme.logoUrl}/>
          ) : <span>{initials}</span>}<div><strong>{organization.name}</strong><small>Client services</small></div></Link>
          <div className="portal-links">{navigation.map((item) => <a href={item.href} key={item.label}>{item.label}</a>)}</div>
          <Link className="portal-button portal-button-small" href={`/c/${organization.slug}/login`}>Client portal</Link>
        </nav>
