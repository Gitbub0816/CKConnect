...
export const accountingExportReports = [
  "workbook",
  "ledger",
  "trial-balance",
  "statements",
...
export const accountingExportFormats = ["xlsx", "csv"] as const;

export type AccountingExportReport = (typeof accountingExportReports)[number];
export type AccountingExportFormat = (typeof accountingExportFormats)[number];
...
function styleSheet(sheet: ExcelJS.Worksheet) {
  ...
  for (const column of sheet.columns) {
    column.width = Math.min(
      34,
      Math.max(12, Number(column.header?.toString().length ?? 12) + 4),
    );
...
export async function buildAccountingExportWorkbook({
  db,
  organization,
  report,
}: {
  ...
  const [accounts, entries, invoices, payments, expenses, vendors, products] =
    await Promise.all([
  ...
  const trialRows = accounts.map((account) => {
    const debitNormal = ["ASSET", "EXPENSE", "COST_OF_GOODS_SOLD"].includes(
      account.type,
    );
    const balance = account.lines.reduce(
      (sum, line) =>
        sum +
        (debitNormal
  ...
  const vendorRows = vendors.map((vendor) => ({
    ...
    balance: vendor.bills.reduce(
      (sum, bill) => sum + money(bill.total) - money(bill.amountPaid),
      0,
    ),
  }));
  ...
  const statementRows = [
    ...
    [
      "Income",
      trialRows
        .filter((row) => ["INCOME", "OTHER_INCOME"].includes(row.type))
        .reduce((sum, row) => sum + row.balance, 0),
    ],
    [
      "Expenses",
      trialRows
        .filter((row) =>
          ["EXPENSE", "COST_OF_GOODS_SOLD", "OTHER_EXPENSE"].includes(row.type),
        )
        .reduce((sum, row) => sum + row.balance, 0),
    ],
  ].map(([statement, amount]) => ({ statement, amount }));

  if (report === "import-template") {
    addImportTemplate(workbook);
  }
  if (["workbook", "ledger"].includes(report)) {
    addRows(
      workbook.addWorksheet("Raw Ledger"),
      [
  ...
  if (["workbook", "trial-balance"].includes(report)) {
    addRows(
      workbook.addWorksheet("Trial Balance"),
      [
  ...
  if (["workbook", "statements"].includes(report)) {
    addRows(
      workbook.addWorksheet("Statements"),
      [
  ...
  if (["workbook", "invoices"].includes(report)) {
    addRows(
  ...
  if (["workbook", "payments"].includes(report)) {
    addRows(
  ...
  if (["workbook", "expenses"].includes(report)) {
    addRows(
  ...
  if (["workbook", "vendors"].includes(report)) {
    addRows(
      workbook.addWorksheet("Vendors"),
      [
        { header: "Name", key: "name" },
  ...
  if (["workbook", "products"].includes(report)) {
    addRows(
...
function addImportTemplate(workbook: ExcelJS.Workbook) {
  addRows(
    workbook.addWorksheet("Chart of Accounts"),
    [
      { header: "Code", key: "code" },
      { header: "Name", key: "name" },
      { header: "Type", key: "type" },
...
export async function serializeAccountingWorkbook({
  workbook,
  format,
}: {
