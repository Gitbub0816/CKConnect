      ...
      <div className="mx-auto max-w-3xl overflow-hidden border border-black/10 bg-[var(--payment-surface)] shadow-[0_28px_90px_rgba(54,40,20,.15)]" style={{borderRadius:"var(--payment-radius)"}}>
        <header className="flex flex-wrap items-center justify-between gap-4 border-b bg-[var(--payment-header)] px-7 py-6 text-white">
          <div className="flex items-center gap-4">
            {theme?.paymentLogoUrl && (
              ...
              <img
                alt=""
                className="size-12 rounded-lg bg-white/10 object-contain p-1"
                src={theme.paymentLogoUrl}
              />
            )}
            <div>
              <div className="text-xs uppercase tracking-[.18em] text-[var(--payment-primary)]">
        ...
        <div className="grid gap-8 p-7 md:grid-cols-[1fr_270px]">
          <section>
            ...
            <div className="mt-7 overflow-hidden rounded-xl border">
              {invoice.items.map((item) => (
                <div
                  className="flex items-start justify-between gap-5 border-b p-4 last:border-0"
                  key={item.id}
                >
                  <div>
          ...
          <aside className="rounded-xl bg-[#f8f5ef] p-5">
            <div className="text-xs font-bold uppercase tracking-[.13em] text-slate-500">
              Bill to
            </div>
            ...
            {paid ? (
              ...
              <div
                style={
                  {
                    "--primary": theme?.paymentPrimaryColor ?? "#c9a033",
                  } as React.CSSProperties
                }
              >
